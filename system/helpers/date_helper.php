...
//date_when by ntheorist (return time in words)
if ( ! function_exists('date_when') )
{
    function date_when( $timestamp = NULL, $base = NULL)
    {
        ...
        $suffix = $end = ($past) ? 'ที่เเล้ว' : 'จากนี้';
        ...
        $periods = array('ปี'=>31536000,'เดีอน'=>2628000,'วัน'=>86400,'ช้่วโมง'=>3600,'นาที'=>60,'วินาที'=>1);
        ...
        {
            case 'month' : 
                $output = $top[1] == 1 ? ( $past ? 'เดีอนที่เเล้ว' : 'เดีอนหน้า' ) : $top[1] . ' เดือน' . $suffix;
                break;
            case 'day' :
                $output = $top[1] == 1 ? ( $past ? 'เมื่อวาน' : 'พรุ่งนี้' ) .', '. $timestr : $top[1] . ' วัน' . $suffix;
                break;
            case 'hour':
                // Calculate in case, for example if yesterday was only 7 hours ago
                $output = date('j',$base) == date('j',$timestamp) ? 'วันนี้, '.$timestr : (( $past ? 'เมื่อวาน' : 'พรุ่วนี้' ) . ', '.$timestr);
                break;
            default : 
                $output = $top[1] .' '. $top[0] . ( $top[1] > 1 ? '' : '' ) . $suffix;
                break;
        }
